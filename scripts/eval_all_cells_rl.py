...
def load_controller(env, train_cfg):
    ctrl = ModelEmbeddedController(env)
    dp = train_cfg.get("deployed_models", {})
...
def run_transit_episode(ctrl, inner, src_xy, dst_xy, safe_z, env_cfg):
    ...
    return {"success": result.success, "failed_at": None if result.success else "transit",
            "transit": result, "descend": None, "ascend": None}
...
def run_three_stage_episode(ctrl, inner, src_xy, dst_xy, safe_z, hover_z, env_cfg):
    ...
    if not transit_result.success:
        return {"success": False, "failed_at": "transit",
                "transit": transit_result, "descend": None, "ascend": None}
    ...
    if not descend_result.success:
        return {"success": False, "failed_at": "descend",
                "transit": transit_result, "descend": descend_result, "ascend": None}
        ...
        for rep in range(n_reps):
            ...
            try:
                if test_mode == "transit_only":
                    result = run_transit_episode(ctrl, inner, src_xy, dst_xy, safe_z, env_cfg)
                else:
                    result = run_three_stage_episode(ctrl, inner, src_xy, dst_xy, safe_z, hover_z, env_cfg)
            except Exception as e:
                result = {
                    "success": False, "failed_at": "EXCEPTION",
                    "error": str(e), "tb": traceback.format_exc(),
                    "transit": None, "descend": None, "ascend": None,
                }

            success = result["success"]
            failed_at = result.get("failed_at", "") or ""

            t_err = f"{result['transit'].error_mm:.1f}mm" if result.get("transit") else "---"
            d_err = f"{result['descend'].error_mm:.1f}mm" if result.get("descend") else "---"
            a_err = f"{result['ascend'].error_mm:.1f}mm" if result.get("ascend") else "---"

            status = "SUCCESS" if success else f"FAIL@{failed_at}"
            print(f"{src_name}->{dst_name:>4} | {rep+1:>4} | {status:>10} | "
            ...
            results.append({
                ...
                "src_xy": src_xy.copy(), "dst_xy": dst_xy.copy(),
                "rep": rep, "success": success, "failed_at": failed_at,
                "result": result,
            })
...
def analyze_results(results):
    ...
    failing_pairs = [(k, v) for k, v in pair_stats.items() if v["successes"] < v["total"]]
    ...
    for (src, dst), stats in failing_pairs[:40]:
        ...
        print(f"  {src}->{dst}: {stats['successes']}/{stats['total']} ({rate:.0%}) "
...
def main():
    ...
    parser.add_argument("--test-mode", choices=["transit_only", "three_stage"], default="three_stage")
    ...
    print(f"Total pairs: {len(pairs)}, reps/pair: {args.n_reps}, total episodes: {len(pairs)*args.n_reps}")
    ...
    ctrl = load_controller(env, train_cfg)
    inner = env.unwrapped

    t_start = time.time()
    results = run_evaluation(pairs, ctrl, inner, env, env_cfg,
                             n_reps=args.n_reps, test_mode=args.test_mode)

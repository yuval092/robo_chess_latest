...
def run_descend_diagnostic(model, env, start_xy, label, max_steps=400):
    ...
    stability_threshold = env_cfg.get("stability_vel_threshold", 0.02)
    eval_drift_limit = env_cfg.get("eval_drift_limit", 0.010)
    ...
    print(f"Arm at start: {grip} (error from target: {np.linalg.norm(grip - start_pos)*1000:.1f}mm)")
    print()
    ...
    for step in range(max_steps):
        ...
        z_tol = env_cfg.get("descend_success_z_tolerance", 0.015)
    ...
    if success:
        ...
        speed = np.linalg.norm(inner._utils.get_site_xvelp(inner.model, inner.data, "robot0:grip"))
...
def main():
    ...
    descend_path = train_cfg.get("deployed_models", {}).get("descend")
    ...
    env_cfg = load_config("env")
    eval_drift = env_cfg.get("eval_drift_limit", 0.010)
    ...
    for xy, label in positions:
        ok, reason = run_descend_diagnostic(model, env, np.array(xy), label, max_steps=400)

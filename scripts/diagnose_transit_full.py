...
import src.chess_env  # noqa: register ChessFetchTask-v0
...
def run_episode_detailed(ctrl, inner, target_xy, safe_z, env_cfg, verbose=False):
    ...
    stability_vel_threshold = env_cfg.get("stability_vel_threshold", 0.02)
    success_threshold = env_cfg.get("success_threshold", 0.010)
    braking_dist = env_cfg.get("braking_dist", 0.010)
    max_steps = 300
    ...
    try:
        for step_idx in range(max_steps):
            grip_pos = inner._utils.get_site_xpos(inner.model, inner.data, "robot0:grip").copy()
            grip_vel = inner._utils.get_site_xvelp(inner.model, inner.data, "robot0:grip").copy()
            ...
            if is_near and is_stable:
                success = True
                steps_log.append({
                    "step": step_idx, "pos": grip_pos.copy(), "vel": grip_vel.copy(),
                    "speed": speed, "d_xy": d_xy, "d_z": d_z, "dist_3d": dist_3d,
                    "is_near": True, "is_stable": True, "in_braking": in_braking_zone,
            ...
            steps_log.append({
                "step": step_idx, "pos": grip_pos.copy(), "vel": grip_vel.copy(),
                "speed": speed, "d_xy": d_xy, "d_z": d_z, "dist_3d": dist_3d,
                "is_near": is_near, "is_stable": is_stable, "in_braking": in_braking_zone,
                "action": action.copy(), "outcome": None
            })
            ...
            # Crash check
            grip_pos_post = inner._utils.get_site_xpos(inner.model, inner.data, "robot0:grip").copy()
    ...
    # Final position
    grip_pos_final = inner._utils.get_site_xpos(inner.model, inner.data, "robot0:grip").copy()
    grip_vel_final = inner._utils.get_site_xvelp(inner.model, inner.data, "robot0:grip").copy()
...
def analyze_timeout_episode(ep):
    ...
    braking_steps = [s for s in log if s["d_xy"] < 0.010]

    print(f"\n  --- Timeout analysis ---")
    print(f"  Target: x={target_pos[0]:.3f}, y={target_pos[1]:.3f}")
    print(f"  Final pos: x={ep['final_pos'][0]:.3f}, y={ep['final_pos'][1]:.3f}, z={ep['final_pos'][2]:.3f}")
    print(f"  Final d_xy={ep['final_d_xy']*1000:.1f}mm, d_z={ep['final_d_z']*1000:.1f}mm, speed={ep['final_speed_mms']:.1f}mm/s")
    ...
    if near_goal_steps:
        ...
        for s in near_goal_steps[-20:]:
            if s["action"] is not None:
                a = s["action"]
                print(f"    step={s['step']:3d}: pos=({s['pos'][0]:.4f},{s['pos'][1]:.4f}), "
    ...
    if osc_steps:
        ...
        if len(osc_steps) > 2:
            ...
            sign_changes_x = sum(1 for i in range(1, len(vx_vals)) if vx_vals[i] * vx_vals[i-1] < 0)
            sign_changes_y = sum(1 for i in range(1, len(vy_vals)) if vy_vals[i] * vy_vals[i-1] < 0)
            print(f"  Velocity sign changes (last 10 steps): X={sign_changes_x}, Y={sign_changes_y}")
            if sign_changes_x + sign_changes_y >= 3:
                print("  CONFIRMED OSCILLATION: frequent velocity direction reversals")


def main():
    parser = argparse.ArgumentParser(description="Full transit diagnostic with per-step logging.")
    parser.add_argument("--n-episodes", type=int, default=30)
    parser.add_argument("--verbose", action="store_true")
    parser.add_argument("--capture-timeouts", type=int, default=5, help="Number of timeout episodes to analyze in detail")
    ...
    transit_model_path = train_cfg.get("deployed_models", {}).get("transit")
    ...
    print(f"Success threshold: {success_threshold*1000:.0f}mm, Stability: {stability_vel_threshold*1000:.0f}mm/s")
    print()
    ...
    print(f"{'EP':>4} | {'Target XY':>20} | {'Outcome':>10} | {'Steps':>6} | {'FinalDxy':>9} | {'FinalSpd':>10}")
    print("-" * 80)
    ...
    for ep in range(args.n_episodes):
        ...
        ep_result = run_episode_detailed(ctrl, inner, target_xy, safe_z, env_cfg, verbose=args.verbose)

        outcome = "SUCCESS" if ep_result["success"] else ep_result["crash_reason"] or "TIMEOUT"
        print(f"{ep+1:>4} | ({target_xy[0]:.3f},{target_xy[1]:.3f}) -> ({inner.goal_pos[0]:.3f},{inner.goal_pos[1]:.3f}) | "
    ...
    print()
    print("=" * 80)
    print(f"Results: {successes}/{args.n_episodes} success, {timeouts} timeouts, {crashes} crashes")
    ...
    for ep in timeout_episodes:
        print(f"  target=({ep['target_pos'][0]:.3f},{ep['target_pos'][1]:.3f}), "
    ...
    if timeout_episodes and success_episodes:
        ...
        home_xy = np.array(env_cfg.get("home_position_xy", [0.88, 0.2641]))
        timeout_dists = [np.linalg.norm(np.array(ep["target_pos"][:2]) - home_xy) for ep in timeout_episodes]
        success_dists = [np.linalg.norm(np.array(ep["target_pos"][:2]) - home_xy) for ep in success_episodes]
        print(f"\nDistance from home (board center):")
        print(f"  Timeouts: avg={np.mean(timeout_dists)*1000:.0f}mm, min={np.min(timeout_dists)*1000:.0f}mm, max={np.max(timeout_dists)*1000:.0f}mm")
        print(f"  Successes: avg={np.mean(success_dists)*1000:.0f}mm, min={np.min(success_dists)*1000:.0f}mm, max={np.max(success_dists)*1000:.0f}mm")

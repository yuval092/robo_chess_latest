...
def main() -> None:
    ...
    def log_step(phase: str) -> None:
        ...
        l_finger = float(env_inner._utils.get_joint_qpos(model, data, "robot0:l_gripper_finger_joint").item())
        r_finger = float(env_inner._utils.get_joint_qpos(model, data, "robot0:r_gripper_finger_joint").item())

        arm_joints = {}
        for jname in ARM_JOINTS:
            try:
                arm_joints[jname] = float(env_inner._utils.get_joint_qpos(model, data, jname).item())
            except Exception:
        ...
        all_pos = {pid: _piece_pos(env_inner, jn) for pid, jn in all_joint_names.items()}
    ...
    for stage_name, stage_res in result.stage_results:
        print(f"  {stage_name}: steps={stage_res.steps} success={stage_res.success} "

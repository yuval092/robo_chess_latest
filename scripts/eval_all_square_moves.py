...
def hide_other_pieces(
    env, teleporter: PieceTeleporter, registry: PieceRegistry, moving_piece_id: str
) -> None:
    """Return hide other pieces."""
...
def selected_pairs(
    from_square: str | None, to_square: str | None, include_same: bool
) -> list[tuple[str, str]]:
    ...
    *,
    ...
    try:
        ...
        controller = ScriptedController(
            env, drift_limit=drift_limit, render_fn=render_fn, render_delay=delay
        )
        ...
        for index, (src, dst) in enumerate(pairs, start=1):
            ...
            if not result.success:
                failure = MoveCheckFailure(
                    src, dst, "move", result.error, format_stages(result.stage_results)
                )
                failures.append(failure)
                print(
                    f"{index}/{len(pairs)} {src}->{dst}: FAIL {failure.error} [{failure.stages}]"
                )
                env.reset()
                hide_other_pieces(inner, teleporter, registry, piece_id)
                home_qpos = robot_home_qpos(inner).copy()
            ...
            if check_home:
                ...
                if not home_result.success:
                    ...
                    failures.append(failure)
                    print(
                        f"{index}/{len(pairs)} {src}->{dst}: FAIL return_home {home_result.error}"
                    )
                    env.reset()
                    hide_other_pieces(inner, teleporter, registry, piece_id)
                    home_qpos = robot_home_qpos(inner).copy()
            ...
            print(
                f"{index}/{len(pairs)} {src}->{dst}: ok max_stage_err={max_error:.1f}mm"
            )
    finally:
        env.close()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Exhaustively test physical moves between board squares."
    )
    parser.add_argument(
        "--piece",
        default="black_rook_a",
        help="Physical piece id to reuse for all checks.",
    )
    parser.add_argument("--from-square", help="Optional source square filter, e.g. a8.")
    parser.add_argument(
        "--to-square", help="Optional destination square filter, e.g. h1."
    )
    parser.add_argument(
        "--include-same", action="store_true", help="Include same-square no-op checks."
    )
    parser.add_argument(
        "--max-cases", type=int, help="Run only the first N generated cases."
    )
    ...
    print(
        f"SUMMARY passed={summary.passed}/{summary.total} failures={len(summary.failures)}"
    )
    for failure in summary.failures:
        print(
            f"FAILURE {failure.src}->{failure.dst} kind={failure.kind} error={failure.error}"
        )

...
def main():
    ...
    c3_xy = bm.square_name_to_xy("c3")
    safe_z = env_cfg["safe_z"]
    home_xy = env_cfg.get("home_position_xy", [0.88, 0.2641])
    ...
    print(f"Finger state: l_finger={l_finger:.5f}, target={inner.finger_target_joint:.5f}")
    ...
    print(f"{'Step':>5} | {'grip_x':>8} {'grip_y':>8} {'grip_z':>8} | "
    ...
    for step in range(max_steps):
        ...
        if is_near and is_stable:
            success = True
            print(f"{step:>5} | {grip_pos[0]:8.4f} {grip_pos[1]:8.4f} {grip_pos[2]:8.4f} | "
    ...
    if not success:
        ...
        print(f"Final d_xy:       {d_xy*1000:.1f}mm  (threshold: {success_threshold*1000:.0f}mm)")
        print(f"Final d_z:        {d_z*1000:.1f}mm  (threshold: {success_threshold*1000:.0f}mm)")
        print(f"Final speed:      {speed*1000:.2f}mm/s  (threshold: {stability_vel_threshold*1000:.0f}mm/s)")
    else:
        grip_pos = inner._utils.get_site_xpos(inner.model, inner.data, "robot0:grip").copy()
        print()
    ...
    print(f"  Scripted transit tol:   4mm (no speed check)")
    print(f"  MAX_STEPS (inference):  {max_steps}")
    print(f"  Training env show_chess_pieces: False → arm starts at RANDOM board pos")
    print(f"  Production show_chess_pieces:   True  → arm starts at HOME_POS={HOME_POS}")

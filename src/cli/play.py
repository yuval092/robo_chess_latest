"""robo-chess-play — start the RoboChess web UI with MuJoCo simulation."""
...
def main():
    """Entry point for robo-chess-play."""
    parser = argparse.ArgumentParser(
        prog="robo-chess-play",
        description=(
            "Start the RoboChess web UI with live MuJoCo simulation. "
            "Opens a browser-accessible chess interface and drives the robot arm "
            "through each move. Press Ctrl-C to stop."
        ),
    )
    parser.add_argument("--host", default="127.0.0.1", help="Flask bind address (default: 127.0.0.1).")
    parser.add_argument("--port", type=int, default=8000, help="Flask bind port (default: 8000).")
    parser.add_argument(
        "--no-visualize",
        dest="visualize",
        action="store_false",

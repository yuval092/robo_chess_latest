...
class LogicalPieceTracker:
    ...
    def reserve_piece_for(self, color: str, piece_type: str) -> str:
        ...
        for piece_id in sorted(self._reserve_to_square):
            if piece_id.startswith(prefix) and self._reserve_to_square[piece_id] is None:
                return piece_id
    ...
    def next_promotion_reserve_slot(self, color: str) -> str:
        ...
        promoted_out = sum(
            1 for piece_id, square in self._piece_to_square.items()
            if piece_id.startswith(f"{color}_pawn_") and square is None and piece_id not in captured_set
        )
...
class MovePlanner:
    ...
    def plan(self, move: chess.Move) -> PhysicalPlan:
        ...
        if captured_piece_id is not None:
            ...
            color = "white" if captured_piece and captured_piece.color == chess.WHITE else "black"
            commands.append(RemoveFromBoardCommand(captured_piece_id, self.tracker.next_graveyard_slot(color)))
        ...
        if move.promotion is not None:
            ...
            commands.append(TeleportCommand(moving_piece_id, "promotion_reserve", reserve_slot))
            commands.append(TeleportCommand(promoted_piece_id, "square", dst))

        return PhysicalPlan(move.uci(), commands)

    def _castle_commands(self, move: chess.Move, king_id: str, king_src: str, king_dst: str) -> list[ArmMoveCommand]:
        ...
        if rook_id is None:
            raise ValueError(f"No physical rook tracked at castling source square {rook_src}")
        return [

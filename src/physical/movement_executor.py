...
class MovementExecutor:
    ...
    def __init__(self, env, controller, board_mapper: BoardMapper, occupancy: PhysicalOccupancy):
    ...
    def move_piece_between_squares(self, piece_id: str, src_square: str, dst_square: str) -> PhysicalMoveResult:
        try:
            self.occupancy.assert_piece_at(piece_id, src_square)
            self.occupancy.assert_square_empty(dst_square)
        except ValueError as exc:
            return PhysicalMoveResult(False, piece_id, src_square, dst_square, [], str(exc))

        src_xy = self.board_mapper.square_name_to_xy(src_square)
        dst_xy = self.board_mapper.square_name_to_xy(dst_square)
        return self.move_piece_xy(piece_id, src_xy, dst_xy, src_square=src_square, dst_square=dst_square)
    ...
    def move_piece_xy(
        ...
        *,
        ...
        if not result.success:
            ...
            return PhysicalMoveResult(False, piece_id, src_square, dst_square, result.stage_results, error)
        ...
        if xy_error > 0.020:
            return PhysicalMoveResult(False, piece_id, src_square, dst_square, result.stage_results, f"XY_RECONCILE_FAILED {xy_error*1000:.1f}mm")
        if z_error > 0.010:
            return PhysicalMoveResult(False, piece_id, src_square, dst_square, result.stage_results, f"Z_RECONCILE_FAILED {z_error*1000:.1f}mm")

        if dst_square is not None:
            placed_xyz = self.board_mapper.square_to_piece_xyz(__import__("chess").parse_square(dst_square))
        ...
        return PhysicalMoveResult(True, piece_id, src_square, dst_square, result.stage_results)

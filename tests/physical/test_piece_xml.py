...
def test_all_active_and_reserve_piece_bodies_exist():
    ...
    for piece in registry.all_pieces():
        assert (
            mujoco.mj_name2id(uw.model, mujoco.mjtObj.mjOBJ_BODY, piece.body_name) != -1
        )
        assert (
            mujoco.mj_name2id(uw.model, mujoco.mjtObj.mjOBJ_JOINT, piece.joint_name)
            != -1
        )
        assert (
            mujoco.mj_name2id(uw.model, mujoco.mjtObj.mjOBJ_GEOM, piece.cube_geom_name)
            != -1
        )
        assert (
            mujoco.mj_name2id(
                uw.model, mujoco.mjtObj.mjOBJ_GEOM, piece.visual_geom_name
            )
            != -1
        )
    ...
    for piece_id, _, _ in reserve_piece_ids():
        assert (
            mujoco.mj_name2id(uw.model, mujoco.mjtObj.mjOBJ_BODY, f"piece_{piece_id}")
            != -1
        )
...
def test_piece_joint_cube_and_visual_properties():
    ...
    for piece in registry.all_pieces():
        joint_id = mujoco.mj_name2id(
            uw.model, mujoco.mjtObj.mjOBJ_JOINT, piece.joint_name
        )
        ...
        cube_id = mujoco.mj_name2id(
            uw.model, mujoco.mjtObj.mjOBJ_GEOM, piece.cube_geom_name
        )
...
def test_each_piece_type_uses_correct_mesh():
    ...
    for piece in registry.all_pieces():
        ...
        actual_mesh_name = mujoco.mj_id2name(
            uw.model, mujoco.mjtObj.mjOBJ_MESH, mesh_id
        )

...
All configuration is loaded via `src/utils/io.py:load_config(name)`, which reads `configs/{name}.yaml` and returns a plain Python dict. There are three configuration files:
...
Required config values are accessed with `cfg["key"]` so missing keys fail loudly. Scenario-specific override lookups are explicit and fall back only to another configured key.

`configs/deployed_models.yaml` contains production inference checkpoint paths. `configs/training.yaml` contains training hyperparameters and the pretrained base model path.

New controller and reconciliation keys include `transit_tolerance_m`, `vertical_tolerance_m`, `step_gain`, `min_step_size_m`, `max_step_size_m`, `transit_max_steps`, `vertical_max_steps`, `grasp_verify_drift_mm`, `rl_max_steps_per_stage`, `reconcile_xy_tolerance_m`, and `reconcile_z_tolerance_m`.

`configs/chess.yaml` now includes `board.validation.*` and `reachability_expected.*` for geometry checks.

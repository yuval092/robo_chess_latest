...
**Mitigation**: `QueuedUIBackend` (in `src/ui/queued_backend.py`) serializes all game API calls through a `queue.Queue`. The main thread (running the MuJoCo viewer loop) drains the queue on every step, processes the request, and puts the result back on a response queue. Flask threads block waiting for their result.
...
**Mitigation**: The value is in `configs/env.yaml` and can be recalibrated with `scripts/eval_grasp_physics.py`.

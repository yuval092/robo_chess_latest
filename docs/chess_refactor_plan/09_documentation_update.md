...
|---|---|---|
| `archive/` deleted | Stage 0 | `00_overview.md` |
| 3 unused XML demo scenes deleted (`push.xml`, `reach.xml`, `slide.xml`) | Stage 1 | `04_scene_generation.md` |
...
| `src/cli/` entry point modules added | Stage 2 | `00_overview.md` |
| All `.get()` fallbacks removed | Stage 3 | `08_configuration.md` |
| New config keys added to `env.yaml` | Stage 3 | `08_configuration.md` |
...
| `eval_targeted.py` updated (config load + sys.path fix) | Stage 7 | `10_evaluation.md` |
...
- Add `eval_targeted.py` with a description of its targeted-square evaluation purpose.
...
- Add `src/cli/` subdirectory with `run_chess_ui.py`, `generate_scene.py`, `visualize.py`.
...
- Remove `push.xml`, `reach.xml`, and `slide.xml` from the file listing. They no longer exist — these were upstream gymnasium-robotics demo scenes that had no references in this project.
- Keep `robot.xml` and `shared.xml` explicitly documented as **required includes**: `pick_and_place.xml` includes both via `<include file="robot.xml">` and `<include file="shared.xml">` tags. These files must not be deleted.
- Update the listing to note `robot.xml` and `shared.xml` as "required includes used by `pick_and_place.xml`".
...
| `rl_max_steps_per_stage` | `300` | Max inference steps for one RL stage |
```
...
**Add a new section: "Config Split"**:
```markdown
## 4. Deployed Models Config

`configs/deployed_models.yaml` contains the production inference model paths, split from
`configs/training.yaml` in Stage 3 of the cleaning refactor.

Production code (`model_controller.py`) loads from `deployed_models.yaml` only.
Training code (`trainer.py`) loads from `training.yaml` only.
Neither config file imports the other.
```
...
- Add `eval_targeted.py` row: targeted square RL evaluation for specific problem squares.
...
**Add Section 11.6: `eval_targeted.py`**:
```markdown
### `eval_targeted.py`

**Purpose**: Targeted RL evaluation for specific problem squares identified during earlier
diagnostics. Runs RL models against a subset of squares (not the full board) to quickly
verify improvements after a new model is deployed.
...
**Config**: Loads deployed model paths from `configs/deployed_models.yaml`.
```
...
Located in `src/utils/env_utils.py`.
...
# 2. push.xml, reach.xml, slide.xml NOT referenced in docs (they're deleted)
grep -r "push\.xml\|reach\.xml\|slide\.xml" docs/current_status/
# Must return nothing (or only references to their deletion)

# 3. robot.xml and shared.xml ARE still documented (required includes)
grep -l "robot\.xml\|shared\.xml" docs/current_status/
# Should show 04_scene_generation.md

# 4. All new entities are documented
...
grep -l "eval_targeted" docs/current_status/

# 5. Configuration doc lists all new keys
grep "transit_tolerance_m\|reconcile_xy_tolerance_m\|required_cell_size_m\|rl_max_steps_per_stage" docs/current_status/08_configuration.md
```
...
After all 10 stages are complete:
...
grep -rn '\.get("' src/chess_env/simulation.py src/chess_env/task.py src/chess_env/model_controller.py
...
# No training/ imports in src/ (Stage 10 complete)
grep -rn "from training\|import training" src/
# Must return nothing

# No old phase9 naming anywhere (Stage 10 complete)
